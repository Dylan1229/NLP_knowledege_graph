...
LIMITED=1839  #设置一次执行最多对多少条新闻进行实体识别
...
def main():
    mongo_client = pymongo.MongoClient('mongodb://192.168.43.52:27017/')  # 连接数据库
    ...
    for coll_name in MONGO_DB_COLLECTIONS:
        with tf.Session(config=config) as sess:
            ...
            articles = list(mongo_coll.find({"eneitys": None}).limit(LIMITED))

            articles_count = len(articles)
            # print(articles_count)
            for article in articles:
                all_abstract = ''
                len_abstract_3 = len(article['abstract_3'])
                demo_sent = all_abstract
                for i in range(0, len_abstract_3):
                    demo_sent = demo_sent + article['abstract_3'][i]
                print(demo_sent)
                #demo_sent = article['abstract_3'][0] + article['abstract_3'][1] + article['abstract_3'][2]
                if demo_sent == '' or demo_sent.isspace():
                    #print('See you next time!')
                    continue
                else:
