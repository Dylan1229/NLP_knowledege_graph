    ...
    if _subjects:
        ...
        for _s in _subjects:
            ...
            for i in des:
                ...
                # 人工特征2：query和实体描述都分词，然后判断query每个词是否在实体描述中出现过（对应一个0/1序列，每个词对应的标记要重复词的长度那么多次，以保证得到通常长度的序列）
...
def test():
    ...
    connect = pymongo.MongoClient(host='192.168.1.103', port=27017)
    mongo_newsdb = connect['finance_data']
    collection = mongo_newsdb['sina_test']
    ...
    for article in articles:
        ...
        i = i + 1
        print("第%d篇文章----------------------------------" % i)
        if 'triple_des' not in article:
            time.sleep(0.1)
            triple_des = {}
            text_in = ""
            for j in article['abstract_3']:
                text_in = text_in + j
            ...
            if text_in is "":
                ...
                continue
            for md in set(extract_items(text_in,article)):
